...
const { addRuleValidator } = require('../../../../src/middleware/validators');
const {
  STRATEGIES,
  ORDER_TYPES,
  COIN_UNITS,
...
describe('addRuleValidator test', () => {
  test('should call next if the rule with all fields is verified', async () => {
    ...
    } = await validationErrorsResult(addRuleValidator)(req, res, next);

    expect(next).toHaveBeenCalledTimes(middlewareCalledTimes);
    expect(validationErrors.isEmpty()).toBe(true);
  });

  test('should call next if the rule with only essential fields is verified', async () => {
    const req = {
      body: {
        strategy: STRATEGIES.SIMPLE_ARBITRAGE,
        coinUnit: COIN_UNITS.BTC,
        currencyUnit: CURRENCY_UNITS.USD,
        orderType: ORDER_TYPES.MARKET_ORDER,
        assetRange: 0.8,
        assetMinLimit: 1000,
  ...
  test('should have errors about essential fields if the rule does NOT exist', async () => {
    ...
    expect(validationErrors.array()).toHaveLength(8);
    expect(validationErrors.array()).toEqual([
      {
        location: 'body',
        param: 'strategy',
        value: undefined,
